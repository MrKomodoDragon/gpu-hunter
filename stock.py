...
text = 'The product {} is found in Stock. View it at {}.'.format(product_name, product_url)
message = 'Subject: {}\n\n{}'.format(subject, text)
...
if re.search(keywords, code2.decode('utf-8'), re.IGNORECASE):
    webhook = DiscordWebhook(url='webhook_url_goes_here', content=f"{product_name} is in stock now at {product_url}")
